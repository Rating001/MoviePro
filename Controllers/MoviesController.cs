...
namespace MoviePro.Controllers
{
    public class MoviesController : Controller
    {
        ...
        {
            //Prevent duplicate imports (Warn user instead of importing it again)
            ...
            //Import: Step 1 (Get the raw data from the API
            var movieDetail = await _tmdbMovieService.MovieDetailAsync(id);

            //Import: Step 2 (Run that raw data through the mapping service
            var movie = await _tmdbMappingService.MapMovieDetailAsync(movieDetail);

            //Import: Step 3 (Add the new movie to the database)
            _context.Add(movie);
            await _context.SaveChangesAsync();

            //Import Step 4 (Add the movie to the default "All" Collection
        ...
        {
            var library = await _context.Movie.ToListAsync();
            return View(library);
        }
